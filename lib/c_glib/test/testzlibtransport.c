...
test_write_fail(void)
{
  ...
  ThriftTransport *transport = NULL;
  int port = 51198;
  gint buf[36] = TEST_DATA; /* a buffer */
  ...
  if ( pid == 0 )
    {
      ...
      g_assert (thrift_zlib_transport_write_end (transport, NULL) == TRUE);
      g_assert (thrift_zlib_transport_write (transport, buf, 36, NULL) == FALSE);
      g_assert (thrift_zlib_transport_flush (transport, NULL) == FALSE);

      thrift_zlib_transport_close (transport, NULL);
      g_object_unref (transport);
      g_object_unref (tsocket);
